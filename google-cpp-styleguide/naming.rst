...
.. tip::
 ...
    尽可能给有描述性的命名，别心疼空间，毕竟让代码易于新读者理解更重要。不要用只有项目开发者能理解的缩写，也不要通过砍掉几个字母来缩写单词。
...
所有类型命名 —— 类, 结构体, 类型别名 (``typedef``), 枚举，模板类型参数 —— 均使用该约定. 例如:
...
常规函数:
 ...
    函数名的每个单词首字母大写, 没有下划线。建议将缩写当做简单单词处理(i.e.StartRpc(), not StartRPC())。
        .. code-block:: c++
...
取值和设值函数:
    ...
    取值（Accessors）和设值（Mutators）函数建议与变量名匹配，也不强制. 这儿摘录一个类, ``num_entries_`` 是该类的实例变量:
...
.. tip::
    ...
    名字空间用小写字母命名, 并基于项目名称和目录结构: ``google_awesome_project``.避免顶层名字空间名之间的冲突，以及嵌套名字空间的名之间的冲突。
    
    顶层命名空间名应与项目或是团队名同名，
    
    避免嵌套空间名与顶层命名空间名冲突，因为名称索引规则，名字空间名之间的冲突会导致意料之外的编译终止。内嵌名字空间名不应为std。选择特别的项目名而不是容易冲突的名字，比如
    (websearch::index, websearch::index_util)优于 websearch::util.
